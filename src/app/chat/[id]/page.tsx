...
const ChatById = ({ params }: { params: { id: string } }) => {
  const session = useSession();
  const { messages, input, handleInputChange, handleSubmit, stop, reload } =
    useChat({
      api: "/api/chat",
    });
  return (
    <div className="flex flex-col h-full">
      <div className="break-word bg-chat-300 flex-grow h-full pt-4 pb-16">
        <ol className="h-4/5 overflow-y-auto">
          {messages.map((m) => {
            return (
...
function useChatHelper(arg0: { api: string }): {
  messages: any;
  input: any;
